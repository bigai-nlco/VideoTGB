...
class LSTP(nn.Module):
    ...
    def generate(
        ...
        try:
            ...
            flow_pixel_values = flow_frames
            of = []
            for flow_frame in flow_pixel_values:
                padder = InputPadder(flow_frame[0].shape)
                flow_frame = padder.pad(flow_frame)
                frames_feats = self.of_extractor(flow_frame[:-1], flow_frame[1:])
            ...
            cand_start_index = [[0]]
            cand_end_index = [[flow_frames.shape[1]-1]]
        # video_lengths = [frames.size(1)]
        # video_lengths = [of.size(1)]
        video_lengths = [flow_frames.size(1)]
        ...
        # ## 1) reshape qformer representation
        ...
        idx = 0
        for ii, width in enumerate([nframe] * batch_size):
        ...
        attention_mask = torch.cat([language_model_attention_mask, text_encoding["attention_mask"]], dim=1)
        inputs_embeddings = self.model.get_input_embeddings()(text_encoding["input_ids"])
...
class LSTP_blip2(nn.Module):
    ...
    def generate(
        ...
        query_tokens = self.model.query_tokens.expand(image_embeddings.shape[0], -1, -1)
        all_query_output = self.model.qformer(
            query_embeds=query_tokens,
            encoder_hidden_states=image_embeddings,
