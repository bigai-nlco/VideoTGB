...
class IVTINSTRUCT(Dataset):
    ...
    def collate(self, batch):
        ...
        frames = [data["frames"] for data in batch if data["frames"] is not None]
        if frames != []:
            frames = torch.cat(frames)
        # frames = torch.cat([data['frames'] for data in batch if data["frames"] is not None]) # B*T, 3, 224, 224
    ...
    def _load_data(self):
        ...
        if self.split == "train":
            inst_path = os.path.join(self.text_dir, "nlp_tune.json")
            with open(inst_path) as jp:
                inst_lst = json.load(jp)
            # data_lst = inst_lst + data_lst
            for dct in inst_lst:
                dct["id"] += len(data_lst) + 1000
                dct["id"] = str(dct["id"])
                data_lst.append(dct)
        # data_lst = []
