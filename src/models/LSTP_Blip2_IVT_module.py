...
class LSTPModule(LightningModule):
    ...
    def __init__(
        ...
        self.model.language_model = get_peft_model(self.model.language_model, self.peft_config)
    ...
    def forward(self, batch):
        ...
        if pixel_values != []:
            ...
            language_model_attention_mask = torch.zeros(
            ...
            for ii, width in enumerate(batch["widths"]):
                if width:
                    ...
                    idx += width
                    language_model_attention_mask[ii] = torch.ones(32, dtype=torch.long, device=query_output.device)
            language_model_inputs = self.model.language_projection(query_output)
            # q_language_model_inputs = self.model.language_projection(query_output)
            ...
            # language_model_attention_mask = torch.zeros((batch_size, 32), dtype=torch.long, device=q_language_model_inputs.device)
            # idx = 0
            # for ii, width in enumerate(batch["widths"]):
            #     if width:
            #         language_model_inputs[ii] = q_language_model_inputs[idx:idx+width].mean(0).reshape(-1, q_language_model_inputs.shape[-1])
            #         language_model_attention_mask[ii] = torch.zeros
            #         idx += width
