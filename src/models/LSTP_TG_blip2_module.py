...
class LSTPSFModule(LightningModule):
    ...
    def __init__(
        ...
        # for tracking results for each epochs
        self.outputs = []
    ...
    def test_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        of_lengths = batch["of_lengths"]
        norm_start_preds = []
        norm_end_preds = []
        for idx in range(len(of_lengths)):
            of_length = of_lengths[idx]
            start_pred = start_logits[idx, :of_length].argmax(dim=-1).item()
            end_pred = end_logits[idx, :of_length].argmax(dim=-1).item()
            # start_pred = start_preds[idx]
            # end_pred = end_preds[idx]
            norm_start_preds.append(start_pred/(of_length-1))
            norm_end_preds.append(end_pred/(of_length-1))

        self.outputs.append({"idxs": batch["idxs"], "start_preds": norm_start_preds, "end_preds": norm_end_preds})

    def on_test_epoch_end(self) -> None:
        """Lightning hook that is called when a test epoch ends."""
        idxs, start_preds, end_preds = list(), list(), list()
        ...
        result = {}
        for idx, start_pred, end_pred in zip(idxs, start_preds, end_preds):
            result[idx] = [start_pred, end_pred]
        
        with open("results/videoinstruct.json", "w") as jp:
            json.dump(result, jp)
